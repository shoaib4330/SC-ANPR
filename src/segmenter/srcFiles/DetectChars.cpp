...
bool loadKNNDataAndTrainKNN(void) {
    ...
    cv::FileStorage fsClassifications("/home/abdul/CLionProjects/segmentation/classifications.xml", cv::FileStorage::READ);        // open the classifications file
    ...
    fsClassifications.release();                                            // close the classifications file
                                                                            ...
                                                                            // read in training images_for_segmentation_tesing ////////////////////////////////////////////////////////////

    cv::Mat matTrainingImagesAsFlattenedFloats;         // we will read multiple images_for_segmentation_tesing into this single image variable as though it is a vector

    cv::FileStorage fsTrainingImages("/home/abdul/CLionProjects/segmentation/images_for_segmentation_tesing.xml", cv::FileStorage::READ);              // open the training images_for_segmentation_tesing file

    if (fsTrainingImages.isOpened() == false) {                                                 // if the file was not opened successfully
        std::cout << "error, unable to open training images_for_segmentation_tesing file, exiting program\n\n";         // show error message
        return(false);                                                                          // and exit program
    }

    fsTrainingImages["images_for_segmentation_tesing"] >> matTrainingImagesAsFlattenedFloats;           // read images_for_segmentation_tesing section into Mat training images_for_segmentation_tesing variable
    fsTrainingImages.release();                                                 // close the traning images_for_segmentation_tesing file

                                                                                // train //////////////////////////////////////////////////////////////////////////////

                                                                                // finally we get to the call to train, note that both parameters have to be of type Mat (a single Mat)
                                                                                // even though in reality they are multiple images_for_segmentation_tesing / numbers
...
std::vector<PossiblePlate> detectCharsInPlates(std::vector<PossiblePlate> &vectorOfPossiblePlates) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
        ...
        preprocess(possiblePlate.imgPlate, possiblePlate.imgGrayscale, possiblePlate.imgThresh);        // preprocess to get grayscale and threshold images_for_segmentation_tesing
...
///////////////////////////////////////////////////////////////////////////////////////////////////
PossiblePlate* detectCharsInPlates(PossiblePlate* plate,std::string imgName) {
    int intPlateCounter = 0;				// this is only for showing steps
    cv::Mat imgContours;
    std::vector<std::vector<cv::Point> > contours;
    cv::RNG rng;
    ...
    // preprocess to get grayscale and threshold images_for_segmentation_tesing
    preprocess(plate->imgPlate, plate->imgGrayscale, plate->imgThresh);

#ifdef SHOW_STEPS
        cv::imshow("5a", plate->imgPlate);
        cv::imshow("5b", plate->imgGrayscale);
        cv::imshow("5c", plate->imgThresh);
    cv::destroyWindow("5a");
    cv::destroyWindow("5b");
    cv::destroyWindow("5c");
#endif	// SHOW_STEPS
        ...
        // upscale size by 60% for better viewing and character recognition
        cv::resize(plate->imgThresh, plate->imgThresh, cv::Size(), 1.6, 1.6);

        // threshold again to eliminate any gray areas
        cv::threshold(plate->imgThresh, plate->imgThresh, 0.0, 255.0, CV_THRESH_BINARY | CV_THRESH_OTSU);

#ifdef SHOW_STEPS
        cv::imshow("5d", plate->imgThresh);
    cv::destroyWindow("5d");
#endif	// SHOW_STEPS

        // find all possible chars in the plate,
        // this function first finds all contours, then only includes contours that could be chars (without comparison to other chars yet)
        std::vector<PossibleChar> vectorOfPossibleCharsInPlate = findPossibleCharsInPlate(plate->imgGrayscale, plate->imgThresh);

#ifdef SHOW_STEPS // to print image contours (contours of each character)
        imgContours = cv::Mat(plate->imgThresh.size(), CV_8UC3, SCALAR_BLACK);
        contours.clear();

        for (int i=0; i< vectorOfPossibleCharsInPlate.size(); i++) {
            contours.push_back(vectorOfPossibleCharsInPlate.at(i).contour);
        }

        printf("length=%d",vectorOfPossibleCharsInPlate.size());
    ...
    //cv::destroyWindow("6");
        ...
        if(vectorOfPossibleCharsInPlate.size()==0)
        {
    ...
    //cv::destroyWindow("7");
        ...
        if (vectorOfVectorsOfMatchingCharsInPlate.size() == 0) {
          ...
          ////  cv::waitKey(0);
#endif	// SHOW_STEPS
            plate->strChars = "";            // set plate string member variable to empty string
        }
    ...
    //cv::destroyWindow("8");
        ...
        // perform char recognition on the longest vector of matching chars in the plate
//       std::string licence_plate_number="";
//        licence_plate_number=recognizeCharsInPlate(plate->imgThresh, vectorOfVectorsOfMatchingCharsInPlate[0]);
//        licence_plate_number=licence_plate_number+"_"+recognizeCharsInPlate(plate->imgThresh, vectorOfVectorsOfMatchingCharsInPlate[1]);
//     plate->strChars=licence_plate_number;


    // code to generate separate segmants
    std:: string Detected_plate_name=imgName;
    std:: string pathToDfolder="/home/abdul/CLionProjects/ClassifierTesting/src/segmenter/output/";
    std:: string command="mkdir -p /home/abdul/CLionProjects/ClassifierTesting/src/segmenter/output/";
    std:: string Path=command+Detected_plate_name;
    ...
    if(vectorOfVectorsOfMatchingCharsInPlate.size()>=2) {

        for (int k = 0; k < vectorOfVectorsOfMatchingCharsInPlate[1].size(); ++k) {
            if (k < 3) {
                cv::rectangle(plate->imgThresh, vectorOfVectorsOfMatchingCharsInPlate[1].at(k).boundingRect,
                              SCALAR_GREEN,
                              2);       // draw green box around the char
                // image of a character to be recognized.
                cv::Mat imgROI = plate->imgThresh(vectorOfVectorsOfMatchingCharsInPlate[1].at(
                        k).boundingRect);                 // get ROI image of bounding rect

                cv::Mat imgROIResized;
                // resize image, this is necessary for char recognition (resize the image of character to be recognized)
                cv::resize(imgROI, imgROIResized, cv::Size(RESIZED_CHAR_IMAGE_WIDTH, RESIZED_CHAR_IMAGE_HEIGHT));

                cv::imshow("character" + oder, imgROIResized);

                imgROIResized = cv::Scalar::all(255) - imgROIResized;

                std::string filestorage_path =
                        pathToDfolder + Detected_plate_name + "/" + std::to_string(oder) + ".png";
                cv::imwrite(filestorage_path, imgROIResized);
                oder++;
            }

        }
        // extract line
        for (int j = 0; j < vectorOfVectorsOfMatchingCharsInPlate[0].size(); ++j) {

            cv::rectangle(plate->imgThresh, vectorOfVectorsOfMatchingCharsInPlate[0].at(j).boundingRect, SCALAR_GREEN,
                          2);       // draw green box around the char
            // image of a character to be recognized.
            cv::Mat imgROI = plate->imgThresh(vectorOfVectorsOfMatchingCharsInPlate[0].at(
                    j).boundingRect);                 // get ROI image of bounding rect
            ...
            std::string filestorage_path = pathToDfolder + Detected_plate_name + "/" + std::to_string(oder) + ".png";
            cv::imwrite(filestorage_path, imgROIResized);
      ...
      //  plate->strChars=recognizeCharsInPlate(plate->imgThresh,vectorOfPossibleCharsInPlate);

#ifdef SHOW_STEPS
//        std::cout << "chars found in plate number " << intPlateCounter << " = " << plate->strChars << ", click on any image and press a key to continue . . ." << std::endl;
//        intPlateCounter++;
//        cv::waitKey(0);
#endif	// SHOW_STEPS
...
std::vector<PossibleChar> findPossibleCharsInPlate(cv::Mat &imgGrayscale, cv::Mat &imgThresh) {
    ...
    std::vector<std::vector<cv::Point> > contours; // contours of each character.
...
bool checkIfPossibleChar(PossibleChar &possibleChar) {
    ...
    // note that we are not (yet) comparing the char to other chars to look for a group
        ...
        if ((possibleChar.boundingRect.area() > MIN_PIXEL_AREA && possibleChar.boundingRect.area() < MAX_PIXEL_AREA)
            &&
            (possibleChar.boundingRect.width > MIN_PIXEL_WIDTH && possibleChar.boundingRect.width < MAX_PIXEL_WIDTH) &&
            (possibleChar.boundingRect.height > MIN_PIXEL_HEIGHT && possibleChar.boundingRect.height < MAX_PIXEL_HEIGHT)
            &&
